...
S1 = imread('sicily1.jpg'); 
S2 = imread('sicily2.jpg');

figure("name", "Separate Images");
...
figure("name", "Mixed Images");
subplot(2, 2, 1), imshow(uint8(X1));
subplot(2, 2, 2), imshow(uint8(X2));

%% ------------------------------------------------------------------------
% 1. Reshape the observed signals to vectors
% 2. Move the mean to zero
% 3. Minimize Variance to get angle for principal components
% 4. Get the U matrix for the unitary transformation
% -------------------------------------------------------------------------
[m, n] = size(X1);
x1 = reshape(X1, m*n, 1);
x2 = reshape(X2, m*n, 1);
...
%% ------------------------------------------------------------------------
% 1. Get the variance from the data. 
% 2. Transform the data, applying U^T and 1/Sigma
% 3. Use the closed form to figure out the best phi. 
% 4. Place the min of the data to be zero. 
% 5. Plot it out. 
